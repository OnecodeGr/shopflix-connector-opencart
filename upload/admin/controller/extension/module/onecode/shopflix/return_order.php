...
class ControllerExtensionModuleOnecodeShopflixReturnOrder extends Controller
{
    ...
    {
        $orders = $this->return_order_model->connector->getNewOrders();
        if (count($orders) == 0)
        {
            return false;
        }
        ...
        if (count($orders_to_accept) && $this->model_extension_module_onecode_shopflix_config->convertOrders())
        {
            try
            {
                $this->return_order_model->accept($orders_to_accept);
            }
            catch (Exception $exception)
            {
    ...
    protected function getList()
    {
        ...
        foreach ($results as $result)
        {
            ...
            $data['orders'][] = [
                ...
                'accept' => ($result['status'] == ReturnOrderInterface::STATUS_ON_THE_WAY_TO_THE_STORE)
                    ? $this->url->link($this->getLink() . '/accept', $order_url_params, true)
                    : false,
                'decline' => ($result['status'] == ReturnOrderInterface::STATUS_ON_THE_WAY_TO_THE_STORE)
